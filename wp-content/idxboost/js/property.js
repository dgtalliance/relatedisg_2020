      ...
      switch (view) {
        ...
        case "map":
          ...
          var miniMap = new google.maps.Map(
            document.getElementById("flex-map-view-ct"),
            {
              zoom: 16,
              center: myLatLng2,
      ...
      if ($(this).find("span").hasClass("active")) {
        ...
        $.ajax({
          ...
          success: function (data) {
            _parent.html("save favorite").removeClass("active");
        ...
        $.ajax({
          ...
          dataType: "json",
          success: function (data) {
            _parent.html("remove").addClass("active");

            //active_modal($('#modal_add_favorities'));
            swal({
              title: "Property added to favorites!",
              text: "You can see your saved properties under your account ",
              type: "success",
              timer: 2000,
              showConfirmButton: false,
            });
...
(function () {
    ...
    $resultSearch.on("click", ".clidxboost-btn-check", function () {
      ...
      if (!$elCheck.hasClass("active")) {
        ...
        if ($favoriteLink.length) {
          // Habrá páginas en donde no exista el boton de 'favoritos, por eso compruebo su existencia'
        ...
        if ($favoriteLink.length) {
          // Habrá páginas en donde no exista el boton de
      ...
      if (!$ulSliderb.hasClass("swiping")) {
        ...
        if ($marginLeftUl == "0px") {
          ...
          if ($ulSliderb.hasClass("created")) {
            ...
            $.each(
              getGallery(
                $wrapSlider.parent().attr("data-mls"),
                $wrapSlider.parent().attr("data-counter")
              ),
              function (i, m) {
                $newImages =
                  $newImages +
  ...
  if ($miniFilters.length) {
    ...
    $miniFilters.find("h4").on("click", function () {
      ...
      if ($theLi.hasClass("cities") && !$citiesList.hasClass("ps-container")) {
        ...
        }, Number($theLi.css("transition-duration").replace("s", "")) *
          1000 *
          2);
      ...
      switch ($nameClass) {
        case "all": // Mostrar el 'All Filter'
          ...
          if ($wrapFilters.width() <= 768) {
            ...
            var $SetScrollTop =
              ...
              Number($wrapFilters.css("margin-top").replace("px", ""));
          ...
          if ($wrapFilters.width() <= 640) {
            $allFilters.css({
              ...
              top:
                $wrapFilters.outerHeight() + $wrapFilters.position().top + "px",
              left: "0px",
              height:
                "calc(100vh - " +
        ...
        default:
          ...
          if ($liClicked.hasClass("active")) {
            ...
            setTimeout(function () {
              $allFilters.removeClass("individual");
            }, Number($allFilters.css("transition-duration").replace("s", "")) *
              1000);
          }
      }
    });
  ...
  $("body").on("click", "#full-slider .wrap-slider button", function () {
    // BOTON NEXT
  ...
  $("body").on("click", "#full-slider .wrap-slider li", function () {
    ...
    if (!$modalTitle.hasClass("worked")) {
      ...
      var datesPropertie = [];
      datesPropertie.push(
        $lisPI.eq(0).text().split(" ")[0],
        $lisPI.eq(1).text().split(" ")[0],
        $lisPI.eq(2).text().split(" ")[0],
        $lisPI.eq(4).text().split(" ")[0]
      );
  ...
  $("body").on(
    "click",
    "#modal_img_propertie .close-modal, #modal_img_propertie .bg-black",
    function () {
  ...
  $("body").on("click", "#modal_img_propertie .nav", function () {
    ...
    if ($(this).hasClass("next")) {
      ...
      if (Number($totalItems.text()) !== Number($actualindex.text())) {
        ...
        $wrapperImg.find(".img").attr(
          "src",
          $fullSlider
            .find("li")
            .eq(Number($actualindex.text()) - 1)
            .find("img")
            .attr("src")
        );
        ...
        $wrapperImg
          .find(".img")
          .attr("src", $fullSlider.find("li").eq(0).find("img").attr("src"));
