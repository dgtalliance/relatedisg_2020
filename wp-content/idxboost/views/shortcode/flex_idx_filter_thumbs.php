<?php if (isset($featured_filter_page) && !empty($featured_filter_page)) : ?>
	...
	<section class="flex-block-description mtop-60" id="featured-section">
		...
		else  $idx_txt_text_tit_property_front  = get_theme_mod('idx_txt_text_tit_property_front'); ?>
		...
		<div class="wrap-result view-grid">
					...
					<?php foreach ($response['items'] as $property) : ?>
						<ul class="result-search idx_color_primary">
							<li data-mls="<?php echo $property['mls_num']; ?>" class="propertie" data-geocode="<?php echo $property['lat']; ?>:<?php echo $property['lng']; ?>" data-class-id="<?php echo $property['class_id']; ?>">
								...
								<?php endif; ?>
								<?php $arraytemp = str_replace(' , ', ', ', $property["address_large"]);
								...
								<?php if ((isset($atts['registration_key']) && !empty($atts['registration_key'])) && isset($atts['agent_slug'])) : ?>
									<a href="<?php echo rtrim(site_url(), "/") . '/' . $atts['agent_slug']; ?>/property/<?php echo $property['slug']; ?>?ibref=<?php echo $atts['registration_key']; ?>" class="view-detail"><?php echo $property['address_large']; ?></a>
								<?php else : ?>
			...
			<?php } else { ?>
				<div class="message-alert info-color" id="box_flex_alerts_msg">
					<p><?php echo __('Please update your Filter ID key on your IDX Boost Dashboard to display live MLS data on your website.', IDXBOOST_DOMAIN_THEME_LANG); ?></p>
				</div>
			<?php } ?>
		</div>
		<?php if (empty(get_theme_mod('idx_txt_text_property_front')))  $idx_txt_text_property_front  = '';
