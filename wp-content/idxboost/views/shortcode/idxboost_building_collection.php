                    ...
                    <div class="gwr">
                      <ul id="sub-filters">
                        ...
                        <li class="clidxboost-icon-arrow-select idxboost_collection_filterviews <?php if ($response['payload']['modo_view'] == 2): ?>list<?php else: ?>grid<?php endif; ?>" id="filter-views">
                          ...
                          <select>
                            <option value="grid" <?php if (empty($view_url)) { selected($response['payload']['modo_view'], 1); }else{ if ($view_url=='grid') echo "selected"; } ?> forview="view_grip">Grid</option>
                            <option value="list" <?php if (empty($view_url)) { selected($response['payload']['modo_view'], 2); }else{ if ($view_url=='list') echo "selected"; }  ?> forview="view_list">List</option>
                          </select>
                          <?php } ?>
                        </li>
                      </ul>
