...
if (!function_exists("idxboost_buyers_form_sc")) {
    function idxboost_buyers_form_sc($atts, $content = null)
    {
...
if (!function_exists("idxboost_rentals_form_sc")) {
    function idxboost_rentals_form_sc($atts, $content = null)
    {
        $atts = shortcode_atts(array(
            'registration_key' => ""
        ), $atts);
...
if (!function_exists("idxboost_sellers_form_sc")) {
    function idxboost_sellers_form_sc($atts, $content = null)
    {
        $atts = shortcode_atts(array(
            'registration_key' => ""
        ), $atts);
...
if (!function_exists('flex_idx_autocomplete_sc')) {
    function flex_idx_autocomplete_sc($atts)
    {
        ...
        wp_localize_script('flex-idx-autocomplete', '__ib_autocomplete', array(
            'ajaxUrl'        => admin_url('admin-ajax.php'),
            'siteUrl'        => site_url(),
            'agent_page_slug' => $atts['agent_page_slug']
        ));
...
if (!function_exists('fb_flex_idx_property_detail_sc')) {
    function fb_flex_idx_property_detail_sc($track_property_view = true)
    {
        global $wp, $wpdb, $flex_idx_info, $flex_idx_lead, $post;

        $has_registration_key = (isset($_GET['ibref']) && !empty($_GET['ibref'])) ? strip_tags($_GET['ibref']) : null;
        ...
        if ("idx-agents" === $post->post_type) {
            list($agentslug, $page, $slug) = $wp_request_exp;
        } else {
        ...
        $sendParams = array(
            ...
            'track_property_view' => (true === $track_property_view ? "yes" : "no"),
            'access_token'     => $access_token,
            'flex_credentials' => $flex_lead_credentials,
            'data'             => array(
...
function insert_fb_in_head()
{
    ...
    */
    ...
    if (is_numeric($postproperty) && $postproperty >= 0) {
        $data = fb_flex_idx_property_detail_sc(false);
...
if (!function_exists('flex_idx_filter_sc')) {
    function flex_idx_filter_sc($atts, $content = null)
    {
        ...
        $has_registration_key = (isset($atts['registration_key']) && !empty($atts['registration_key'])) ? sanitize_text_field($atts['registration_key']) : null;
        $access_token          = flex_idx_get_access_token($has_registration_key);
            ...
            if (isset($response['info']['property_type'])) {
                ...
                foreach ($exp_property_type as $ptype) {
                    if (in_array($ptype, $list_ptypes)) {
                        $catch_list_ptype[] = ["value" => $ptype, "label" => (isset($list_ptypes[$ptype]) ? $list_ptypes[$ptype] : $ptype)];
                    }
                }
...
if (!function_exists('flex_idx_buildind_sc')) {
    function flex_idx_buildind_sc($atts)
    {
        ...
        if (
            !empty($response) && is_array($response) && count($response) > 0 &&
