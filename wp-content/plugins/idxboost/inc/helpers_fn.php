...
if (!function_exists('is_agent_custompage')) {
    function is_agent_custompage()
    {
        $current_path = isset($_SERVER['REQUEST_URI']) ? trim($_SERVER['REQUEST_URI'], '/') : "/";
        ...
        if (count($current_path_exp) >= 3) {
            if ('property' === $current_path_exp[1]) {
                $agent_custompage = $current_path_exp[0];
            }
        }

        return $agent_custompage;
    }
}
...
if (!function_exists('flex_idx_get_info')) {
    function flex_idx_get_info()
    {
        ...
        if (false !== $idxboost_agent_info) {
            ...
            // overwrite force registration settings if navigating an agent page
            $agent_custompage = is_agent_custompage();

            if (null !== $agent_custompage) {
                $agent_registration_key = $wpdb->get_col($wpdb->prepare("
                SELECT t2.meta_value
                FROM {$wpdb->posts} t1
                INNER JOIN {$wpdb->postmeta} t2
                ON t1.ID = t2.post_id
                WHERE t1.post_type = 'idx-agents'
                AND t1.post_status = 'publish'
                AND t1.post_name = %s
                AND t2.meta_key = '_flex_agent_registration_key'
                LIMIT 1
                ", $agent_custompage));

                $request = wp_remote_post(FLEX_IDX_API_AGENT_FETCH_CUSTOM_SETTINGS, [
                    'method' => 'POST',
                    'timeout' => 45,
                    'blocking' => true,
                    'headers' => [],
    ...
    function flex_idx_connect_fn()
    {
        ...
        if (isset($response['active_agents']) && is_array($response['active_agents'])) {
            ...
            foreach ($response['active_agents'] as $idx_agent) {
                ...
                update_post_meta($wp_idx_agent, '_flex_agent_full_name', $idx_agent['full_name']);
                ...
                update_post_meta($wp_idx_agent, '_flex_agent_modified_in', $idx_agent['modified_in']);
            }
...
if (!function_exists('flex_idx_request_property_form_fn')) {
    function flex_idx_request_property_form_fn()
    {
        ...
        $lead_credentials = isset($_COOKIE['ib_lead_token']) ? ($_COOKIE['ib_lead_token']) : '';
        $recaptcha_response = isset($_POST["recaptcha_response"]) ? trim(strip_tags($_POST["recaptcha_response"])) : "";
        $registration_key = isset($_POST['registration_key']) ? strip_tags($_POST['registration_key']) : "";
...
if (!function_exists('flex_idx_register_assets')) {
    function flex_idx_register_assets()
    {
        ...
        wp_localize_script('flex-idx-search-commercial-v2', '__flex_idx_search_filter_v2', array(
            ...
            'lookupAutocomplete' => IDX_BOOST_AUTOCOMPLETE_SERVICE,
        ...
        wp_localize_script('flex-auth-check', '__flex_g_settings', array(
            ...
            'autocomplete' => array('board' => $flex_idx_info['board_id'], 'service_url' => IDX_BOOST_AUTOCOMPLETE_SERVICE),
...
if (!function_exists('flex_idx_admin_render_tools_page')) {
    function flex_idx_admin_render_tools_page()
    {
        ...
        /*create table*/
        $sql = "
        CREATE TABLE IF NOT EXISTS `${tbl_idxboost_tools}` (
