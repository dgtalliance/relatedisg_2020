...
Handlebars.registerHelper("propertyPermalinkOpen", function (slug) {
  if (typeof IB_AGENT_REGISTRATION_KEY !== "undefined") {
    ...
    //return __flex_idx_filter_regular.propertyDetailPermalink + "/" + slug + "?ibref=" + encodeURIComponent(IB_AGENT_REGISTRATION_KEY);
  } else {
...
Handlebars.registerHelper("propertyPermalink", function (slug) {
  if (
    ...
    "undefined" !== typeof IB_AGENT_REGISTRATION_KEY
  ) {
    return (
      IB_AGENT_PROPERTY_SLUG +
      "/" +
...
Handlebars.registerHelper("idxSliderLoop", function (property) {
  ...
  // @todo antony check modal for sold or validate
  // if (("undefined" !== typeof property) && property.hasOwnProperty("gallery")) {
  ...
  if (property.gallery.length > 0) {
    ...
    if (count == 1) {
      sliderItems.push(
        '<img class="ib-pvsitem" src="https://www.idxboost.com/i/default_thumbnail.jpg">'
      );
      sliderItems.push(
        '<img class="ib-pvsitem" src="https://www.idxboost.com/i/default_thumbnail.jpg">'
      );
...
function markPropertyAsFavorite(mlsNumber, element, from) {
  var markPropertyAsFavoriteEndpoint = __flex_idx_filter_regular.trackListingsDetail.replace(
    /{{mlsNumber}}/g,
    mlsNumber
  );
    ...
    success: function (response) {
      ...
      if ("modal" === from) {
        if ("add" === response.type) {
          ...
          $("#wrap-list-result ul#result-search")
            .find("li.propertie")
            .each(function () {
              if ($(this).data("mls") == mlsNumber) {
                $(this).find(".clidxboost-icon-check").addClass("active");
                $(this)
                  .find(".flex-favorite-btn")
                  .data("alert-token", response.token_alert);
...
if ("undefined" === typeof loadPropertyInModal) {
  function loadPropertyInModal(mlsNumber) {
    ...
    var viewListingDetailEndpoint = __flex_idx_filter_regular.lookupListingsDetail.replace(
      /{{mlsNumber}}/g,
      mlsNumber
    );
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (IB_MODAL_WRAPPER.length && IB_MODAL_TPL.length) {
          ...
          if (
            response.hasOwnProperty("oh") &&
            response.oh == "1" &&
            response.hasOwnProperty("oh_info")
          ) {
          ...
          if ("share" in navigator) {
            ...
            if ("undefined" !== typeof IB_AGENT_PROPERTY_SLUG) {
              history.pushState(
                null,
                null,
                IB_AGENT_PROPERTY_SLUG + "/" + response.slug
              );
            } else {
              history.pushState(
                null,
...
if (IB_MODAL_WRAPPER.length) {
  ...
  IB_MODAL_WRAPPER.on("click", ".ib-pbtnopen", function () {
    ...
    if (
      typeof is_references_active != "undefined" &&
      is_references_active == "yes"
    ) {
  ...
  IB_MODAL_WRAPPER.on("click", ".ib-pbtnclose", function () {
    ...
    if ("share" in navigator) {
      ...
      if ("undefined" !== typeof IB_AGENT_URL) {
        if ("" === urlParams.toString()) {
          history.pushState(null, null, IB_AGENT_URL + "listings");
        } else {
          history.pushState(
            null,
            null,
            IB_AGENT_URL + "listings" + "?" + urlParams.toString()
          );
        }
      } else {
        if ("" === urlParams.toString()) {
  ...
  $(".ib-property-share-friend-f").on("submit", function (event) {
    ...
    var shareWithFriendEndpoint = __flex_idx_filter_regular.shareWithFriendEndpoint.replace(
      /{{mlsNumber}}/g,
      mlsNumber
    );
  ...
  IB_MODAL_WRAPPER.on("submit", ".ib-propery-inquiry-f", function (event) {
    ...
    var mlsNumber = $(this).find("input[name='mls_number']:eq(0)").val();
    var requestInformationEndpoint = __flex_idx_filter_regular.requestInformationEndpoint.replace(
      /{{mlsNumber}}/g,
      mlsNumber
    );
