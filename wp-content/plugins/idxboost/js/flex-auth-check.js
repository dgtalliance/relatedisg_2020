        ...
        jQuery.ajax({
          ...
          success: function (response) {
            ...
            if (response.lead_info.first_name.length) {
              fill_first_letter_name_values.push(
                response.lead_info.first_name.charAt(0)
              );
            }

            if (response.lead_info.last_name.length) {
              fill_first_letter_name_values.push(
                response.lead_info.last_name.charAt(0)
              );
            }

            jQuery(".ib-lead-first-letter-name").html(
              fill_first_letter_name_values.join("")
            );

            if (
              response.lead_info.hasOwnProperty("photo_url") &&
              response.lead_info.photo_url.length
            ) {
            ...
            jQuery(".ib-lead-fullname").html(
              response.lead_info.first_name + " " + response.lead_info.last_name
            );
            jQuery(".ib-lead-firstname").html(
              "Hello " + response.lead_info.first_name + "!"
            );

            jQuery(".ib-agent-fullname").html(
              response.agent_info.first_name +
                " " +
                response.agent_info.last_name
            );
            jQuery(".ib-agent-phonenumber").html(
              response.agent_info.phone_number
            );
            jQuery(".ib-agent-phonenumber").attr(
              "href",
              "tel:" + response.agent_info.phone_number.replace(/[^\d]/g, "")
            );
            jQuery(".ib-agent-emailaddress").attr(
              "href",
              "mailto:" + response.agent_info.email_address
            );
            jQuery(".ib-agent-photo-thumbnail-wrapper").empty();
            jQuery(".ib-agent-photo-thumbnail-wrapper").append(
              '<img src="' + response.agent_info.photo_url + '">'
            );
            ...
            if (response.lead_info.listing_views.length) {
              ...
              for (var i = 0, l = lead_listing_views.length; i < l; i++) {
                ...
                lead_listing_views_html.push(
                  '<img src="' + lead_listing_views[i].thumbnail + '">'
                );
                ...
                lead_listing_views_html.push(
                  "<span>" + lead_listing_views[i].bed + " Beds</span>"
                );
                lead_listing_views_html.push(
                  "<span>" + lead_listing_views[i].bath + " Baths</span>"
                );
                lead_listing_views_html.push(
                  "<span>" + lead_listing_views[i].sqft + " SqFt</span>"
                );
                lead_listing_views_html.push("</div>");
                lead_listing_views_html.push("</div>");
                //console.log(lead_listing_views[i].mls_num);
                lead_listing_views_html.push(
                  '<div class="ms-property-actions">'
                );
                lead_listing_views_html.push(
                  '<button data-mls="' +
                    lead_listing_views[i].mls_num +
                    '" class="ib-la-hp ms-delete"><span>Delete</span></button>'
                );
                lead_listing_views_html.push("</div>");
              ...
              jQuery("#_ib_lead_activity_rows").html(
                lead_listing_views_html.join("")
              );
            }
            ...
            if (response.lead_info.listing_views_pagination.total_pages > 1) {
              ...
              if (response.lead_info.listing_views_pagination.has_prev_page) {
                lead_listing_views_paging.push(
                  '<a class="ib-pagprev ib-paggo" data-page="' +
                    (response.lead_info.listing_views_pagination.current_page -
                      1) +
                    '" href="#"></a>'
                );
              }
              ...
              var lead_listing_views_page_range =
                response.lead_info.listing_views_pagination.page_range_links;

              for (
                var i = 0, l = lead_listing_views_page_range.length;
                i < l;
                i++
              ) {
                if (
                  lead_listing_views_page_range[i] ==
                  response.lead_info.listing_views_pagination.current_page
                ) {
                  lead_listing_views_paging.push(
              ...
              jQuery("#_ib_lead_activity_pagination").html(
                lead_listing_views_paging.join("")
              );
            }
          },
        });
  ...
  if ("yes" === __flex_g_settings.anonymous) {
    if ("undefined" !== typeof IB_PAGE_PROPERTY_DETAIL_MLS_NUMBER) {
      ...
      if (
        -1 ===
        jQuery.inArray(
          IB_PAGE_PROPERTY_DETAIL_MLS_NUMBER,
...
function idx_auto_save_building(lead_data) {
  ...
  if (
    ...
    if (
      ...
      idxboostCollecBuil.success != false
    ) {
      var search_count = 0;
      var name_alert = "";
      var query_filter = "";
      ...
      if (document.location.hash == "#!for-rent") {
        ...
        query_filter =
          idxboostCollecBuil.payload.filter_query + " and is_rental=1 ";
      } else {
        search_count = idxboostCollecBuil.payload.properties.sale.count;
        name_alert = $(".idx_name_building").val() + " for sale";
        query_filter =
          idxboostCollecBuil.payload.filter_query + " and is_rental=0 ";
      }
...
socket = new Pusher(__flex_g_settings.pusher.app_key, {
  ...
  encrypted: true,
  authEndpoint:
    __flex_g_settings.socketAuthUrl +
    "?ib_lead_token=" +
  ...
  $.fn.calculatemortgage = function () {
    ...
    $.ajax({
      ...
      data: {
        ...
        purchase_price: $(".purchase_price_txt")
          .val()
          .replace(/,/g, "")
          .replace(/\./g, "")
          .replace("$", ""),
  ...
  $(function () {
    ...
    $(document).on("submit", "#flex-idx-property-form", function (event) {
      ...
      $.ajax({
        ...
        success: function (data) {
          //data.message
          $("#modal_properties_send .body_md .ico_ok").text(
    ...
    $(document).on("click", ".show-modal", function () {
      ...
      if (typeof mapImg != "undefined") {
        $("#min-map")
          .css("background-image", "url('" + mapImg + "')")
          .removeAttr("data-map-img");
      }
    });
    ...
    */
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success === true) {
                ...
                success: function (response) {
                  ...
                  if (
                    ...
                    jQuery(".ib-lead-first-letter-name").css({
                      "background-color": "transparent",
                      "background-image":
                        "url(" + response.lead_info.photo_url + ")",
                      "background-repeat": "no-repeat",
                      "background-size": "contain",
                      "background-position": "center center",
                      "text-indent": "-9999px",
                  ...
                  jQuery(".ib-lead-fullname").html(
                    response.lead_info.first_name +
                      " " +
                      response.lead_info.last_name
                  );
                  jQuery(".ib-lead-firstname").html(
                    "Hello " + response.lead_info.first_name + "!"
                  );
                  ...
                  jQuery(".ib-agent-phonenumber").attr(
                    "href",
                    "tel:" +
                      response.agent_info.phone_number.replace(/[^\d]/g, "")
                  );
                  jQuery(".ib-agent-emailaddress").attr(
                    "href",
                    "mailto:" + response.agent_info.email_address
                  ...
                  // build pagination
                  if (
                    response.lead_info.listing_views_pagination.total_pages > 1
                  ) {
                    var lead_listing_views_paging = [];

                    if (
                      response.lead_info.listing_views_pagination.has_prev_page
                    ) {
                      lead_listing_views_paging.push(
                        '<a class="ib-pagprev ib-paggo" data-page="' +
                          (response.lead_info.listing_views_pagination
                            .current_page -
                            1) +
                          '" href="#"></a>'
                      );
                    ...
                    if (
                      response.lead_info.listing_views_pagination.has_next_page
                    ) {
                      lead_listing_views_paging.push(
                        '<a class="ib-pagnext ib-paggo" data-page="' +
                          (response.lead_info.listing_views_pagination
            ...
            Cookies.set(
              "_ib_last_logged_in_username",
              response.last_logged_in_username
            );
            ...
            if (
              typeof lastOpenedProperty !== "undefined" &&
              lastOpenedProperty.length
            ) {
              // track listing view
              $.ajax({
                type: "POST",
                ...
                data: {
                  ...
                  mls_number:
                    typeof lastOpenedProperty === "undefined"
                      ...
                      : lastOpenedProperty,
                  mls_opened_list:
                    Cookies.get("_ib_user_listing_views") === "undefined"
                      ? []
                      : JSON.parse(Cookies.get("_ib_user_listing_views")),
                },
                success: function (response) {
            ...
            localStorage.setItem(
              "idxboost_credential",
              JSON.stringify(objectCredential)
            );
    ...
    $(document).ready(function () {
      ...
      if (resetPassPath.length == 2) {
        ...
        $("#formReset #reset_email").attr(
          "placeholder",
          word_translate.enter_you_new_password
        );
        ...
        success: function (response) {
          ...
          if (true === response.success) {
            ...
            if (response.message == "Logged in succesfully.") {
              ...
              ib_log_message =
                word_translate.your_account_has_been_created_successfully;
            }
            ...
            if (
              ("undefined" !== typeof IB_IS_SEARCH_FILTER_PAGE &&
                true === IB_IS_SEARCH_FILTER_PAGE) ||
              ("undefined" !== typeof IB_IS_REGULAR_FILTER_PAGE &&
                true === IB_IS_REGULAR_FILTER_PAGE)
            ) {
    ...
    $(".header-tab a").click(function () {
      ...
      switch (tabId) {
        ...
        case "tabRegister":
          ...
          $(".text-slogan").text(
            word_translate.join_to_save_listings_and_receive_updates
          );
          $(".modal_cm.social_items .footer_md").show();
          $("#modal_login").removeClass("tabResetHidden");
          break;
    ...
    $(".modal_cm .close").click(function () {
      $(
        "#modal_login, #overlay_modal, #modal_add_favorities, #modal_properties_send"
      ).removeClass("active_modal");
    ...
    $(document).on("click", ".show_modal_login_active", function () {
      ...
      if (!$(".menu_login_active").hasClass("active_login")) {
        $(".menu_login_active")
          .removeClass("disable_login")
          .addClass("active_login");
      } else {
        $(".menu_login_active")
          .removeClass("active_login")
          .addClass("disable_login");
      }
    });
      ...
      switch ($selectLanguage) {
        ...
        case "zh-TW":
          $($(".goog-te-menu-frame"))
            ...
            .each(function () {
              if (
                $(this).html() == "chino (tradicional)" ||
                $(this).html() == "chino (Tradicional)" ||
                $(this).html() == "Chinese (Traditional)" ||
                $(this).html() == "hinese (traditional)" ||
                $(this).html() == "中國（繁體）"
              ) {
                $(this).click();
              }
      ...
      switch (cookieLanguage) {
        case "us":
          ...
          $("#available-languages")
            ...
            .addClass("flag-english");
          $(
            ".languages-list .item-languages[data-iso='" + cookieLanguage + "']"
          ).addClass("active");
          break;
        case "ru":
          $("#available-languages").find(".languages-text").text("RU");
    ...
    $("body").mouseup(function (e) {
      if (
        !$(".available-languages-content").is(e.target) &&
        $(".available-languages-content").has(e.target).length === 0
      ) {
...
(function ($) {
  ...
  function msShowMap() {
    ...
    var miniMap = new google.maps.Map(
      document.getElementById("ms-map-result"),
      {
        zoom: 16,
        center: myLatLng2,
        styles: style_map,
...
(function ($) {
  ...
  $(function () {
    $("#result-search").on("click", ".show-modal-properties", function (event) {
      ...
      if (
        "result-search" === _propertie.parent().attr("id") &&
        !$(event.target).hasClass("clidxboost-icon-arrow-select") &&
        !$(event.target).hasClass("flex-favorite-btn")
      ) {
    ...
    $(document).on(
      "click",
      ".overlay_modal_closer_pt, .close-modal-pt",
      function () {
        if ($('button[data-id="modal_login"]:eq(0)').is(":hidden")) {
          return;
        }
        ...
        $("#" + idModal).removeClass("active_modal");
        $("body")
          .removeClass("modal-property-active")
          .css({ "overflow-y": "auto" });
        $("html").removeClass("modal_mobile");
      }
    );
...
(function ($) {
  ...
  if ($ibWgrid.length) {
    ...
    $ibWgrid.on("scroll", function () {
      ...
      let $finalScroll =
        $(".ib-gheader").height() +
        $(".ib-cproperties").height() -
        $(this).height();

      if ($("#drift-widget").length) {
        let $theBottom = Number(
          $("#drift-widget").css("bottom").replace("px", "")
        );
  ...
  $(document).on(
    "change",
    "#lead_submission_sell_form .ib-firadio",
    function () {
...
(function ($) {
  ...
  if ($flexIdxSearchFilter.length) {
    if ($(window).width() >= 1024) {
      let heightMenos =
        $(".ib-filter-container").height() + $("#header").outerHeight();
      if ($(".gwr-breadcrumb").length)
        heightMenos += $(".gwr-breadcrumb").height();
      $flexIdxSearchFilter.css(
        "height",
        "calc(100vh - " + (heightMenos + 2) + "px)"
      );
    }
  }
