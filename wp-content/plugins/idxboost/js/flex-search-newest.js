        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                for (var i = 0, l = items.length; i < l; i++) {
                    ...
                    if (item.status == 1) {
                        if (item.is_favorite) {
                            listingHTML.push('<button class="clidxboost-btn-check flex-favorite-btn" data-alert-token="' + item.token_alert + '" aria-label="Remove Favorite"><span class="clidxboost-icon-check active"></span></button>');
                        } else {
        ...
        if (flex_autocomplete_inner.length) {
            flex_autocomplete_inner.autocomplete({
                source: function(request, response) {
                  $.ajax({
                      url: __flex_g_settings.autocomplete.service_url,
                      dataType: "json",
                      data: { term: request.term, board: __flex_g_settings.autocomplete.board },
