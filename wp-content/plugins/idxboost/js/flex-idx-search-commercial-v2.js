...
(function ($) {
  ...
  Handlebars.registerHelper("idxPermalink", function (property) {
    return (
      __flex_idx_search_filter_v2.propertyDetailPermalink + "/" + property.slug
    );
  });
  ...
  function buildSearchFilterForm() {
    ...
    if (IB_LB_TYPES_OPTIONS.length) {
      IB_LB_TYPES_OPTIONS.each(function (index, node) {
        for (
          var i = 0, l = __flex_idx_search_filter_v2.commercial_types.length;
          i < l;
          i++
        ) {
          var li = document.createElement("li");
          ...
          input.setAttribute(
            "value",
            __flex_idx_search_filter_v2.commercial_types[i].value
          );
          input.classList.add("ib-icheck");
          var label = document.createElement("label");
          label.classList.add("ib-clabel");
          ...
          var text_label_trans = "";
          if (
            __flex_idx_search_filter_v2.commercial_types[i].label == "Homes"
          ) {
            text_label_trans = word_translate.homes;
          } else if (
            __flex_idx_search_filter_v2.commercial_types[i].label ==
            "Condominiums"
          ) {
      ...
      IB_RENTAL_TYPE.on("click", "div", function (event) {
        ...
        switch (rentalType) {
          case "rent":
            ...
            var currentValueMinRent = $(
              ".ib-comm-price-rent-inner-min:eq(0)"
            ).val();

            if (currentValueMinRent.length) {
              currentValueMinRent = currentValueMinRent.replace(
                /[^0-9+]/gi,
                ""
              );
            }
            ...
            var currentValueMaxRent = $(
              ".ib-comm-price-rent-inner-max:eq(0)"
            ).val();

            if (currentValueMaxRent.length) {
              currentValueMaxRent = currentValueMaxRent.replace(
                /[^0-9+]/gi,
                ""
              );
            }
          ...
          case "sale":
            ...
            var currentValueMinsale = $(
              ".ib-comm-price-sale-inner-min:eq(0)"
            ).val();

            if (currentValueMinsale.length) {
              currentValueMinsale = currentValueMinsale.replace(
                /[^0-9+]/gi,
                ""
              );
            }
            ...
            var currentValueMaxSale = $(
              ".ib-comm-price-sale-inner-max:eq(0)"
            ).val();

            if (currentValueMaxSale.length) {
              currentValueMaxSale = currentValueMaxSale.replace(
                /[^0-9+]/gi,
                ""
              );
            }
