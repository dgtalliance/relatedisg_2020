      ...
      idxboost_filter_forms.each(function () {
        ...
        if (inputs.length) {
          inputs.on("change", function () {
            ...
            console.log(
              "change value form filter form #" +
                $(this).parent().data("filter-form-id")
            );
...
(function ($) {
  ...
  $(function () {
    ...
    $("#filter-save-search").on("click", function () {
      ...
      if (__flex_g_settings.anonymous === "yes") {
        ...
        sweetAlert(
          word_translate.oops,
          word_translate.you_cannot_save_search_with_more_than_500_properties,
          "error"
        );
        return;
      } else {
    ...
    $("body").on("touchmove", ".slider-generator .propertie", function (evt) {
      ...
      if (Math.abs(xDiff) > Math.abs(yDiff)) {
        // si se mueve derecha o izquierda
        evt.preventDefault();
        if (xDiff > 0) {
          // izquierda
    ...
    if ($theFilters.length) {
        ...
        switch ($nameClass) {
          case "all": // Mostrar el 'All Filter'
            ...
            if (!$allFilters.hasClass("visible")) {
              // lo pongo asi, x siacaso yá esté visible individualmente y no se oculte, sino, muestre todos
              $allFilters.addClass("visible");
            } else {
              if (
                $allFilters.hasClass("individual") &&
                $allFilters.hasClass("visible")
              ) {
                // Está visible, pero individualmente, le quitaré eso...
                ...
                if (
                  ...
                  // Está visible, y sin individual, lo ocultaré...
            ...
            if ($wrapFilters.width() <= 640) {
              $allFilters.css({
                // porque la cabezera de los filtros están en una sola linea.
                top:
                  $wrapFilters.outerHeight() +
                  $wrapFilters.position().top +
                  "px",
                left: "0px",
                height:
                  "calc(100vh - " +
                  ($wrapFilters.outerHeight() +
                    $theFilters.find("li.save").outerHeight()) +
                  "px)",
              });
            } else if (
              $wrapFilters.width() > 640 &&
              $wrapFilters.width() <= 959
            ) {
              // mayor a 640 pero menor a 768 pixeles de ancho.
              if (!$allFilters.hasClass("neighborhood")) {
                // si no estoy en 'neighborhood' tengo todo el ancho de la pantalla.
                $allFilters.css({
                  // porque la cabezera de los filtros están en 2 lineas.
                  left: "0px",
                  top: $wrapFilters.outerHeight() + "px",
                  height:
                    "calc(100vh - " +
                    ($wrapFilters.outerHeight() + $applyFilters.outerHeight()) +
                    "px)",
                });
              } else {
                // estoy en 'neighborhood', lo aparesco diferente;
                $allFilters.removeAttr("style");
                console.log(
                  "Widt all filter: " +
                    $allFilters.width() +
                    " | position left clicked: " +
                    $liClicked.position().left +
                    " | Li clicked widht: " +
                    $liClicked.width()
                );
                $allFilters.css({
              ...
              // cuando la cabezera de los filtros se muestran en 1 sola linea y el 'all filter' debe ser modal (de 960px para arriba).
              $allFilters.removeAttr("style");
          ...
          default:
            ...
            if ($liClicked.hasClass("active")) {
              // activo la flecha del LI clickeado
              $miniFilter.addClass("visible").siblings().removeClass("visible"); //  aparesco el 'Mimi filter'
              if (!$allFilters.hasClass("individual")) {
                // agrego la 'individualidad' solo si se viene de 'All filter', xq sino, yá la tiene.
              ...
              setTimeout(function () {
                ...
                $allFilters.css("transition-duration").replace("s", "")
              ) * 1000);
            }
        }
      });
      ...
      $(document).on("mouseup", function (e) {
        if ($allFilters.hasClass("visible")) {
          if (
            !$wrapFilters.is(e.target) &&
            $wrapFilters.has(e.target).length === 0
          ) {
  ...
  if ($viewFilter.length) {
    ...
    $viewFilter.on("click", "li", function () {
      ...
      switch ($(this).attr("class").split(" ")[0]) {
        case "grid":
          $(".idxboost-content-filter-" + currentfiltemid + " ")
            .find($wrapResult)
            .removeClass("view-list view-map")
            .addClass("view-grid");
          $cuerpo.removeClass("view-list view-map");
        ...
        case "list":
          $(".idxboost-content-filter-" + currentfiltemid + " ")
            .find($wrapResult)
            .removeClass("view-grid view-map")
            .addClass("view-list");
          $cuerpo.addClass("view-list").removeClass("view-map");
          /*Desactivando scrollResultados(false)*/
          break;
      ...
      if (estado) {
        if (!$viewFilter.find("ul").length) {
          ...
          $viewFilter.find("option").each(function () {
            $(this).replaceWith(
              '<li class="' + $(this).val() + '">' + $(this).text() + "</li>"
            );
          });
  ...
  if ($neighborhoodMenu.width() >= 700) {
    // no pongo 768, xq en MAC: (1280), el espacio para el '$neighborhoodMenu' es solo de 720
    ...
    setTimeout(function () {
      ...
      if (nEnlacesNbh) {
        ...
        var $anchoFinalUl =
          anchoUlNbh +
          Number($enlacesNbh.eq(0).css("margin-right").replace("px", "")) *
            (nEnlacesNbh - 1);
        ...
        if ($anchoFinalUl < $neighborhoodMenu.width()) {
          ...
          if ($porcentaje >= 65) {
            // si el porcentaje del ancho de los items sumados es mayor al 65%, damos flex
          ...
          $nextItemNbh.on("click", function () {
            ...
            $ulNbhMenu.css(
              ...
              "-" +
                ($menuNbhWidth -
                  $lastItemMenuNbh.position().left +
                  $lastItemMenuNbh.width()) +
                "px"
            );
  ...
  if (typeof $showModal != "undefined") {
    $showModal.on("click", function () {
      ...
      var $modalImg = $("#" + $idModal)
        .find(".lazy-img")
        .attr("data-src"); //Consultamos si existe una imagen para mostrar en el Modal
  ...
  function itemActivo(losLi) {
    // refactorizar esto (nueva idea para la función).
  ...
  function creaScrollTemporal(creador, objetivo) {
    if (!objetivo.hasClass("ps-container")) {
      ...
      }, Number(creador.css("transition-duration").replace("s", "")) *
        1000 *
        2);
    }
  }

  function dgt_rangeSlide(
    elementRange,
    minr,
    maxr,
    stepTo,
  ...
  $(function () {
    ...
    $("#result-search, .result-search").on("click", ".view-detail", function (
      ...
      if (
        $(this).hasClass("is-sold-listings") ||
        "undefined" !== typeof IB_IS_AGENT_HOME
      ) {
        return true;
      }
      e.preventDefault();
    ...
    $("#result-search, .result-search").on(
      "click",
      ".flex-favorite-btn",
      function (event) {
  ...
  function initialize() {
    ...
    if (
      "undefined" !== typeof style_map_idxboost &&
      "" !== style_map_idxboost
    ) {
    ...
    map = new google.maps.Map(document.getElementById("code-map"), {
      center: new google.maps.LatLng(25.76168, -80.19179),
      ...
      styles: style_map,
      gestureHandling:
        "1" == __flex_g_settings.is_mobile ? "greedy" : "cooperative",
    });
  ...
  function handleMouseUp(event) {
    ...
    for (var i = 0, l = theArrayOfLatLng.length; i < l; i++) {
      arrayForPolygonSearch.push(
        theArrayOfLatLng[i].lat() + " " + theArrayOfLatLng[i].lng()
      );
    }
  ...
  function filter_refresh_search() {
    ...
    var idxboostnavresult =
      ".idxboost-content-filter-" + currentfiltemid + " #nav-results";
    var idxboostresult =
      ".idxboost-content-filter-" + currentfiltemid + " #result-search";
        ...
        success: function (response) {
          if ("yes" === __flex_g_settings.anonymous) {
            ...
            localStorage.setItem(
              "IB_SAVE_FILTER_PAYLOAD",
              JSON.stringify(buildObjectFilter)
            );
          }
          /*
                    $("html, body").animate({
          ...
          $("#info-subfilters").html(
            word_translate.showing +
              ...
              word_translate.of +
              " " +
              _.formatPrice(response.counter) +
              " " +
          ...
          for (var i = 0, l = items.length; i < l; i++) {
            ...
            var st = ("undefined" === typeof al[1]) ? "" : al[1].replace(/[\d\s]/g, "");
            var final_address = item.address_short + " " + al[0] + ", " + st;
            var final_address_parceada =
              item.address_short + " <span>" + al[0] + ", " + al[1] + "</span>";

            var final_address_parceada_new =
              " <span>" +
              item.address_short.replace(/# /, "#") +
              ", " +
              al[0] +
              ", " +
              al[1] +
            ...
            if (view_grid_type == "1") {
              listingHTML.push(
                '<h2 title="' +
                  item.full_address +
                  '"><span>' +
                  item.full_address_top +
                  "</span><span>" +
                  item.full_address_bottom +
                  "</span></h2>"
              );
            } else {
              listingHTML.push(
            ...
            listingHTML.push(
              '<li class="address">' + final_address_parceada_new + "</li>"
            );
            listingHTML.push(
              '<li class="price">$' +
                _.formatPrice(item.price) +
                text_is_rental +
                "</li>"
            );
            if (item.reduced == "") {
              ...
              listingHTML.push(
                '<li class="pr down">' + item.reduced + "%</li>"
              );
            } else {
            ...
            if (item.development !== "") {
              listingHTML.push(
                '<li class="development"><span>' +
                  item.development +
                  "</span></li>"
              );
            } else if (item.complex !== "") {
              listingHTML.push(
                '<li class="development"><span>' + item.complex + "</span></li>"
              );
            } else {
              listingHTML.push(
                '<li class="development"><span>' +
            ...
            if (item.hasOwnProperty("status")) {
              if (item.status != '2') {
                ...
                if (item.is_favorite) {
                  listingHTML.push(
                    '<button class="clidxboost-btn-check flex-favorite-btn"><span class="clidxboost-icon-check active"></span></button>'
                  );
                } else {
                  listingHTML.push(
                    '<button class="clidxboost-btn-check flex-favorite-btn"><span class="clidxboost-icon-check"></span></button>'
                  );
                }
            ...
            if (is_recent_sales == "yes") {
              if (
                typeof IB_AGENT_PROPERTY_SLUG !== "undefined" &&
                item.status == "2"
              ) {
                listingHTML.push(
                  '<a href="' +
                    ...
                    '" class="view-detail is-sold-listings">' +
                    final_address_parceada_new +
                    "</a>"
                );
              } else {
                listingHTML.push(
                  '<a href="' +
                    flex_idx_filter_params.propertyDetailPermalink +
                    "/" +
                    item.slug +
                    '" class="view-detail">' +
                    final_address_parceada_new +
              ...
              if (typeof IB_AGENT_PROPERTY_SLUG !== "undefined") {
                listingHTML.push(
                  '<a href="' +
                    IB_AGENT_PROPERTY_SLUG +
                    "/" +
                ...
                listingHTML.push(
                  '<a href="' +
                    __flex_g_settings.propertyDetailPermalink +
                    "/" +
                    item.slug +
                    '" class="view-detail">' +
                    final_address_parceada_new +
            ...
            listingHTML.push(
              '<a class="view-map-detail" data-geocode="' +
                item.lat +
                ":" +
                item.lng +
                '">View Map</a>'
            ...
            if (idx_oh == "1" && item.hasOwnProperty("oh_info")) {
              ...
              if (
                ...
                listingHTML.push(
                  '<div class="ms-open"><span class="ms-wrap-open"><span class="ms-open-title">Open House</span><span class="ms-open-date">' +
                    oh_info.date +
                    '</span><span class="ms-open-time">' +
                    oh_info.timer +
                    "</span></span></div>"
                );
              }
          ...
          $(idxboostresult)
            .html(listingHTML.join(""))
            .ready(function () {
              idxboostTypeIcon();
              if (typeof ppchack === "function") {
                ppchack();
              }
              if (typeof idxboost_Hackedbox_cpanel === "function") {
                idxboost_Hackedbox_cpanel();
              }
            });
          ...
          if (paging.total_pages_count > 1) {
            paginationHTML.push(
              '<span id="indicator">' +
                ...
                word_translate.of +
                " " +
                _.formatPrice(paging.total_pages_count) +
                "</span>"
            );
            if (paging.has_prev_page && paging.total_pages_count > 1) {
              paginationHTML.push(
                '<a href="#" data-page="1" title="First Page" id="firstp" class="ad visible">'
              );
              paginationHTML.push(
            ...
            if (paging.has_prev_page) {
              paginationHTML.push(
                '<a href="#" data-page="' +
                  ...
                  '" title="Prev Page" id="prevn" class="arrow clidxboost-icon-arrow-select prevn visible">'
              );
            ...
            for (var i = 0, l = paging.range.length; i < l; i++) {
              ...
              if (paging.current_page_number === loopPage) {
                paginationHTML.push(
                  '<li class="active"><a href="#" data-page="' +
                    loopPage +
                    '">' +
                    loopPage +
                    "</a></li>"
            ...
            if (paging.has_next_page) {
              paginationHTML.push(
                '<a href="#" data-page="' +
                  (paging.current_page_number + 1) +
                  '" title="Prev Page" id="nextn" class="arrow clidxboost-icon-arrow-select nextn visible">'
              );
            ...
            if (paging.has_next_page && paging.total_pages_count > 1) {
              paginationHTML.push(
                '<a href="#" data-page="' +
                  paging.total_pages_count +
                  '" title="First Page" id="lastp" class="ad visible">'
              );
              paginationHTML.push(
                '<span class="clidxboost-icon-arrow-select"></span>'
              );
          ...
          if (
            response.info.min_price != null &&
            response.info.max_price != null
          ) {
            ...
            idxboost_price_filter =
              "price=" + minFilPrice + "~" + maxFilPrice + "&";
            if (inifil_default != 4)
              $("#text-price").html(
                _.formatShortPrice(minFilPrice) +
                  word_translate.to +
                  " " +
                  _.formatShortPrice(maxFilPrice)
              );
            else $("#text-price").html(word_translate.price_range);
          }

          if (
            response.info.min_bedrooms != null &&
            response.info.max_bedrooms != null
          )
            if (
              response.info.min_bedrooms != "--" &&
              response.info.max_bedrooms != "--"
            ) {
              idxboost_bed_filter =
                "bed=" +
                response.info.min_bedrooms +
          ...
          if (
            response.info.min_baths != null &&
            response.info.max_baths != null
          )
            if (
              response.info.min_baths != "--" &&
              response.info.max_baths != "--"
            ) {
              idxboost_bath_filter =
                "bath=" +
                response.info.min_baths +
          ...
          if (response.info.min_year != null && response.info.max_year != null)
            if (
              response.info.max_year != "--" &&
              response.info.max_year != "--"
            ) {
              idxboost_year_filter =
                "yearbuilt=" +
                response.info.min_year +
                "~" +
                response.info.max_year +
                "&";
            } else if (
          ...
          if (
            response.info.min_living_size != null &&
            response.info.max_living_size != null
          )
            if (
              response.info.min_living_size != "--" &&
              response.info.max_living_size != "--"
            )
              idxboost_sqft_filter =
                "sqft=" +
                response.info.min_living_size +
          ...
          if (response.order != null && response.view != null)
            idxboost_pagenum_filter =
              "pagenum=" + response.pagination.current_page_number;

          if (
            response.info.waterfront_option != null &&
            response.info.waterfront_option != "--"
          )
            idxboost_water_filter =
              "waterdesc=" + response.info.waterfront_option + "&";

          if (
            response.info.parking_option != null &&
            response.info.parking_option != "--"
          )
            idxboost_parking_filter =
              "parking=" + response.info.parking_option + "&";

          if (
            response.info.min_lot_size != null &&
            response.info.max_lot_size != null
          )
            if (
              response.info.min_lot_size != "--" &&
              response.info.max_lot_size != "--"
            )
              idxboost_lotsize_filter =
                "lotsize=" +
                response.info.min_lot_size +
          ...
          if (inifil_default != 4) {
            var finurl = $("#idx-filter-min-btn_" + currentfiltemid).data(
              "permalinko"
            );
            var flex_idx_new_url =
              finurl +
              "?" +
              idxboost_price_filter +
              idxboost_bed_filter +
              ...
              idxboost_pagenum_filter;
            if (
              response.hasOwnProperty("only_count") &&
              true === response.only_count
            ) {
              var flex_filter_heading = $(
                "#flex-idx-filter-heading_" + currentfiltemid
              );
              var flex_filter_heading_tpl = flex_filter_heading.data("heading");
              flex_filter_heading_tpl = flex_filter_heading_tpl.replace(
                /\{\{count\}\}/,
                _.formatPrice(response.counter)
              );
              flex_filter_heading_tpl = flex_filter_heading_tpl.replace(
                /\{\{rental\}\}/,
                response.info.rental_type == 1 ? " For Rent " : " For Sale "
              );
              flex_filter_heading.find("h4").html(flex_filter_heading_tpl);
              $("#idx-filter-min-btn_" + currentfiltemid).data(
                "permalink",
                flex_idx_new_url
              );
          ...
          $(idxboost_filter_class + " #flex-idx-search-form").data(
            "save_count",
            response.counter
          );
          $(idxboost_filter_class + " #flex-idx-search-form").data(
          ...
          $(idxboost_filter_class + " #flex-idx-search-form").data(
            "current_page",
            response.pagination.current_page_number
          );
          ...
          $("#paginator-cnt").show();
          jQuery("#form-save .list-check .flex-save-type-options").removeAttr(
            "disabled"
          );
          // reset scroll
  ...
  function idxboost_Hackedbox_cpanel() {
    var ib_object = jQuery(
      "#result-search li.propertie, .result-search li.propertie"
    );
    if (idxboost_hackbox_filter.status)
      if (idxboost_hackbox_filter.result.content != "")
        if (ib_object.length < 3)
          ib_object
            .eq(ib_object.length - 1)
            .after(
              '<li class="propertie button_properties"><div class="ppc-content ppc-video-box">' +
  ...
  function setupMarkers(properties) {
    ...
    infoWindow.addListener("domready", function () {
      $(".ib-load-property-iw").on("click", function (event) {
        if (typeof IB_AGENT_IS_SOLD_LISTINGS !== "undefined") {
          return true;
        }
        event.preventDefault();
        event.stopPropagation();
        ...
        $("#modal_property_detail .detail-modal").html(
          '<span class="ib-modal-property-loading">Loading property details...</span>'
        );
        ...
        $.ajax({
          ...
          data: {
            ...
            agent_slug:
              "undefined" === typeof IB_AGENT_PROPERTY_SLUG
                ? null
                : IB_AGENT_PROPERTY_SLUG,
          },
    ...
    if (properties != null) {
      ...
      for (i = 0; i < unique_properties.length; i++) {
        ...
        google.maps.event.addListener(
          marker,
          "click",
          handleMarkerClick(marker, property, map)
        );
        google.maps.event.addListener(
          marker,
          "mouseover",
    ...
    return function () {
      if (property.group.length > 1) {
        ...
        infobox_content.push(
          '<span class="build">' + property.group.length + "</span>"
        );
        infobox_content.push(
          '<button class="closeInfo"><span>' +
        ...
        for (var i = 0, l = property.group.length; i < l; i++) {
          ...
          infobox_content.push(
            '<h2 title="' +
              property_group.address_short.replace(/# /, "#") +
              '"><span>' +
              property_group.address_short.replace(/# /, "#") +
              "</span></h2>"
          );
          ...
          infobox_content.push(
            '<li class="address"><span>' +
              property_group.address_large.replace(/ , /, ", ") +
              "</span></li>"
          );
          infobox_content.push(
            '<li class="price">$' +
          ...
          infobox_content.push(
            '<li class="living-size"> <span>' +
              ...
              "</span> SF<span>(" +
              property_group.living_size_m2 +
              " m2)</span></li>"
          );
          infobox_content.push(
            '<li class="price-sf"><span>$' +
              property_group.price_sqft +
              " </span>/ SF<span>($" +
              property_group.price_sqft_m2 +
              " m2)</span></li>"
          );
          infobox_content.push("</ul>");
          ...
          infobox_content.push(
            '<img title="' +
              ...
              property_group.address_large.replace(/ , /, ", ") +
              '" src="' +
              property_group.gallery[0] +
              '">'
          );
          infobox_content.push("</div>");
          if ("undefined" !== typeof IB_AGENT_PROPERTY_SLUG) {
            infobox_content.push(
              '<a class="ib-load-property-iw" data-mls="' +
        ...
        infobox_content.push(
          '<li class="living-size"> <span>' +
            ...
            "</span> SF<span>(" +
            property.item.living_size_m2 +
            " m2)</span></li>"
        );
        infobox_content.push(
          '<li class="price-sf"><span>$' +
            property.item.price_sqft +
            " </span>/ SF<span>($" +
            property.item.price_sqft_m2 +
            " m2)</span></li>"
        );
        infobox_content.push("</ul>");
        infobox_content.push('<div class="mapviwe-img">');
  ...
  $(function () {
    ...
    if (flex_pagination.length) {
      flex_pagination.on("click", "a", function (event) {
        ...
        $(
          ".nav-results-" + currentfiltemid + " ul#principal-nav li"
        ).removeClass("active");
        $(
          ".nav-results-" +
            currentfiltemid +
            " ul#principal-nav #page_" +
            currentPage
        ).addClass("active");
        // history.pushState(null, '', $(this).attr('norefre'));
        $(".flex-idx-filter-form-" + currentfiltemid + " #idx_page").val(
          currentPage
        );
        // do ajax
        filter_refresh_search();
    ...
    function update_view_page(evento) {
      ...
      $(
        "#nav-results ul#principal-nav #page_" + $(evento).attr("page")
      ).addClass("active");
      history.pushState(null, "", $(evento).attr("norefre"));
      console.log($(evento).attr("norefre"));
    ...
    $wrapListResult.on(
      ...
      _.debounce(function () {
        ...
        if (
          $("#filter-views").find(".map:eq(0)").hasClass("active") &&
          has_next_page == true
        ) {
  ...
  var year_built_slider_values = _.pluck(
    search_metadata.year_built_range,
    "value"
  );
  var price_slider_values = _.pluck(search_metadata.price_sale_range, "value");
  var price_rent_slider_values = _.pluck(
    search_metadata.price_rent_range,
    "value"
  );
  var baths_slider;
  var beds_slider;
    ...
    if (price_rent_slider.length) {
      price_rent_slider.slider({
        ...
        slide: function (event, ui) {
          ...
          $("#price_rent_from").val(
            "$" + _.formatPrice(price_rent_slider_values[startValue])
          );
          $("#price_rent_to").val(
            "$" + _.formatPrice(price_rent_slider_values[endValue])
          );
        ...
        create: function (event, ui) {
          var min_val =
            $("#idx_min_rent_price").val() === "--"
              ? price_rent_slider_values[0]
              : parseInt($("#idx_min_rent_price").val(), 10);
          var max_val =
            $("#idx_max_rent_price").val() === "--"
              ? price_rent_slider_values[price_rent_slider_values.length - 1]
              : parseInt($("#idx_max_rent_price").val(), 10);
          var _self = $(this);
          var startValue =
            $("#idx_min_rent_price").val() === "--"
              ? 0
              : price_rent_slider_values.indexOf(min_val);
          var endValue =
            $("#idx_max_rent_price").val() === "--"
              ? price_rent_slider_values.length - 1
              : price_rent_slider_values.indexOf(max_val);
    ...
    if (price_slider.length) {
      price_slider.slider({
        ...
        slide: function (event, ui) {
          ...
          var endValue = ui.values[1];
          $("#price_from").val(
            "$" + _.formatPrice(price_slider_values[startValue])
          );
          $("#price_to").val(
            "$" + _.formatPrice(price_slider_values[endValue])
          );
        ...
        create: function (event, ui) {
          var min_val =
            $("#idx_min_price").val() === "--"
              ? price_slider_values[0]
              : parseInt($("#idx_min_price").val(), 10);
          var max_val =
            $("#idx_max_price").val() === "--"
              ? price_slider_values[price_slider_values.length - 1]
              : parseInt($("#idx_max_price").val(), 10);
          var _self = $(this);
          var startValue =
            $("#idx_min_price").val() === "--"
              ? 0
              : price_slider_values.indexOf(min_val);
          var endValue =
            $("#idx_max_price").val() === "--"
              ? price_slider_values.length - 1
              : price_slider_values.indexOf(max_val);
          _self.slider("values", [startValue, endValue]);
    ...
    if (baths_slider.length) {
      baths_slider.slider({
        ...
        slide: function (event, ui) {
          ...
          console.log(
            "From %s to %s",
            baths_slider_values[startValue],
            baths_slider_values[endValue]
          );
        },
        create: function (event, ui) {
          var min_val =
            $("#idx_min_baths").val() === "--"
              ? baths_slider_values[0]
              : parseInt($("#idx_min_baths").val(), 10);
          var max_val =
            $("#idx_max_baths").val() === "--"
              ? baths_slider_values[baths_slider_values.length - 1]
              : parseInt($("#idx_max_baths").val(), 10);
          var _self = $(this);
          var startValue =
            $("#idx_min_baths").val() === "--"
              ? 0
              : baths_slider_values.indexOf(min_val);
          var endValue =
            $("#idx_max_baths").val() === "--"
              ? baths_slider_values.length - 1
              : baths_slider_values.indexOf(max_val);
          _self.slider("values", [startValue, endValue]);
        },
        change: function (event, ui) {
          var startValue = ui.values[0];
          var endValue = ui.values[1];
          var initialStartValue =
            baths_slider_values[startValue] == baths_slider_values[0]
              ? "--"
              : baths_slider_values[startValue];
          var initialEndValue =
            baths_slider_values[endValue] ==
    ...
    if (beds_slider.length) {
      beds_slider.slider({
        ...
        slide: function (event, ui) {
          ...
          console.log(
            "From %s to %s",
            beds_slider_values[startValue],
            beds_slider_values[endValue]
          );
        },
        create: function (event, ui) {
          var min_val =
            $("#idx_min_beds").val() === "--"
              ? beds_slider_values[0]
              : parseInt($("#idx_min_beds").val(), 10);
          var max_val =
            $("#idx_max_beds").val() === "--"
              ? beds_slider_values[beds_slider_values.length - 1]
              : parseInt($("#idx_max_beds").val(), 10);
          var _self = $(this);
          var startValue =
            $("#idx_min_beds").val() === "--"
              ? 0
              : beds_slider_values.indexOf(min_val);
          var endValue =
            $("#idx_max_beds").val() === "--"
              ? beds_slider_values.length - 1
              : beds_slider_values.indexOf(max_val);
          _self.slider("values", [startValue, endValue]);
        },
        change: function (event, ui) {
          var startValue = ui.values[0];
          var endValue = ui.values[1];
          var initialStartValue =
            beds_slider_values[startValue] == beds_slider_values[0]
              ? "--"
              : beds_slider_values[startValue];
          var initialEndValue =
            beds_slider_values[endValue] ==
    ...
    if (sqft_slider.length) {
      sqft_slider.slider({
        ...
        slide: function (event, ui) {
          ...
          var endValue = ui.values[1];
          $("#living_from").val(
            _.formatPrice(sqft_slider_values[startValue]) + " SF"
          );
          $("#living_to").val(
            _.formatPrice(sqft_slider_values[endValue]) + " SF"
          );
        ...
        create: function (event, ui) {
          var min_val =
            $("#idx_min_sqft").val() === "--"
              ? sqft_slider_values[0]
              : parseInt($("#idx_min_sqft").val(), 10);
          var max_val =
            $("#idx_max_sqft").val() === "--"
              ? sqft_slider_values[sqft_slider_values.length - 1]
              : parseInt($("#idx_max_sqft").val(), 10);
          var _self = $(this);
          var startValue =
            $("#idx_min_sqft").val() === "--"
              ? 0
              : sqft_slider_values.indexOf(min_val);
          var endValue =
            $("#idx_max_sqft").val() === "--"
              ? sqft_slider_values.length - 1
              : sqft_slider_values.indexOf(max_val);
          _self.slider("values", [startValue, endValue]);
    ...
    if (lotsize_slider.length) {
      lotsize_slider.slider({
        ...
        slide: function (event, ui) {
          ...
          var endValue = ui.values[1];
          $("#land_from").val(
            _.formatPrice(lotsize_slider_values[startValue]) + " SF"
          );
          $("#land_to").val(
            _.formatPrice(lotsize_slider_values[endValue]) + " SF"
          );
        ...
        create: function (event, ui) {
          var min_val =
            $("#idx_min_lotsize").val() === "--"
              ? sqft_slider_values[0]
              : parseInt($("#idx_min_lotsize").val(), 10);
          var max_val =
            $("#idx_max_lotsize").val() === "--"
              ? sqft_slider_values[sqft_slider_values.length - 1]
              : parseInt($("#idx_max_lotsize").val(), 10);
          var _self = $(this);
          var startValue =
            $("#idx_min_lotsize").val() === "--"
              ? 0
    ...
    if (year_built_slider.length) {
      year_built_slider.slider({
        ...
        create: function (event, ui) {
          var min_val =
            $("#idx_min_year").val() === "--"
              ? year_built_slider_values[0]
              : parseInt($("#idx_min_year").val(), 10);
          var max_val =
            $("#idx_max_year").val() === "--"
              ? year_built_slider_values[year_built_slider_values.length - 1]
              : parseInt($("#idx_max_year").val(), 10);
          var _self = $(this);
          var startValue =
            $("#idx_min_year").val() === "--"
              ? 0
              : year_built_slider_values.indexOf(min_val);
          var endValue =
            $("#idx_max_year").val() === "--"
              ? year_built_slider_values.length - 1
              : year_built_slider_values.indexOf(max_val);
          _self.slider("values", [startValue, endValue]);
  ...
  $(function () {
    ...
    if (sort_options.length) {
      sort_options.on("change", function () {
        ...
        // update hidden form
        $(".flex-idx-filter-form-" + currentfiltemid + " #idx_sort").val(
          current_sort
        );
