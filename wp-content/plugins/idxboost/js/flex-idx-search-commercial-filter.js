  ...
  function handleMarkerClick(marker, property, map) {
    return function () {
      ...
      if (property.group.length > 1) {
        ...
        for (var i = 0, l = property.group.length; i < l; i++) {
          ...
          infobox_content.push(
            '<li class="ib-ibditem ib-ibbeds"><span class="ib-ibdbold">' +
              property_group.property_class_name +
              "</span></li>"
          );
          infobox_content.push(
            '<li class="ib-ibditem ib-ibbaths"><span class="ib-ibdbold">' +
              _.formatPrice(property_group.lot_size) +
              "</span> Lot Size</li>"
          );
        ...
        infobox_content.push(
          '<li class="ib-ibditem ib-ibbeds"><span class="ib-ibdbold">' +
            property.item.property_class_name +
            "</span></li>"
        );
        infobox_content.push(
          '<li class="ib-ibditem ib-ibbaths"><span class="ib-ibdbold">' +
            _.formatPrice(property.item.lot_size) +
            "</span> Lot Size</li>"
        );
    ...
    if (ib_m_types.length) {
      ...
      for (var i = 0, l = ib_search_filter_dropdown.length; i < l; i++) {
        ...
        if ("Single Family Homes" == option.label) {
          ...
          "Commercial / Business / Agricultural / Industrial Land" ==
          option.label
        ) {
          text_translate =
            word_translate.commercial_business_agricultural_industrial_land;
  ...
  function attachListenersMobileForm() {
    ...
    ib_min_price.on({
      ...
      keyup: function () {
        ...
        if (currentValue.length) {
          ...
          this.value =
            this.value != "" && this.value > 0 ? _.formatPrice(this.value) : "";
        }
      },
    });
    ...
    ib_min_bsize.on({
      change: function () {
        ...
        IB_SEARCH_FILTER_FORM.find('[name="min_building_size"]').val(
          currentValue
        );
        IB_SEARCH_FILTER_FORM.find('[name="page"]').val(1);
    ...
    ib_max_bsize.on({
      change: function () {
        ...
        IB_SEARCH_FILTER_FORM.find('[name="max_building_size"]').val(
          currentValue
        );
        IB_SEARCH_FILTER_FORM.find('[name="page"]').val(1);
    ...
    */
    ...
    if (IB_LB_TYPES_OPTIONS.length) {
      IB_LB_TYPES_OPTIONS.each(function (index, node) {
        for (
          var i = 0, l = __flex_idx_search_filter.commercial_types.length;
          i < l;
          i++
        ) {
          var li = document.createElement("li");
          ...
          input.setAttribute(
            "value",
            __flex_idx_search_filter.commercial_types[i].value
          );
          input.classList.add("ib-icheck");
          var label = document.createElement("label");
          label.classList.add("ib-clabel");
          ...
          if (__flex_idx_search_filter.commercial_types[i].label == "Homes") {
            ...
            __flex_idx_search_filter.commercial_types[i].label == "Condominiums"
          ) {
            text_label_trans = word_translate.condominiums;
          } else if (
            __flex_idx_search_filter.commercial_types[i].label == "Townhouses"
          ) {
            text_label_trans = word_translate.townhouses;
          } else if (
            __flex_idx_search_filter.commercial_types[i].label ==
            "Single Family Homes"
          ) {
            ...
            "Agricultural" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.agricultural;
          } else if (
            "Apartments" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.apartments;
          } else if (
            "Automotive" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.automotive;
          } else if (
            "Building" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.building;
          } else if (
            "Business" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.business;
          } else if (
            "Church" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.church;
          } else if (
            "Commercial" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            ...
            "Condominium" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.condominium;
          } else if (
            "Dock" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.dock;
          } else if (
            "Dock Height" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.dock_height;
          } else if (
            "Duplex / Quad Plex / Triplex" ==
            __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.duplex_quad_plex_triplex;
          } else if (
            "Franchise" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            ...
            "Hotel" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.hotel;
          } else if (
            "Income" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.income;
          } else if (
            "Industrial" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            ...
            "Medical" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.medical;
          } else if (
            "Multifamily" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.multifamily;
          } else if (
            "Office" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.office;
          } else if (
            "Professional" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            ...
            "Restaurant" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.restaurant;
          } else if (
            "Retail" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.retail;
          } else if (
            "School" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.school;
          } else if (
            "Service" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            ...
            "Showroom" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.showroom;
          } else if (
            "Special" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.special;
          } else if (
            "Store" == __flex_idx_search_filter.commercial_types[i].label
          ) {
            text_label_trans = word_translate.store;
          } else if (
            "Warehouse" == __flex_idx_search_filter.commercial_types[i].label
          ) {
  ...
  function handleFilterSearchLookup(event) {
    ...
    ib_xhr_handler = $.ajax({
      ...
      success: function (response) {
        ...
        IB_SEARCH_FILTER_FORM.find('[name="lot_size_measure_type"]').val(
          params.lot_size_measure_type
        );
        ...
        if (IB_LB_TYPES_OPTIONS.length) {
          IB_LB_TYPES_OPTIONS.find("input").each(function (index, node) {
            var currentValue =
              -1 === $.inArray(node.value, params.property_type)
        ...
        if (ib_comm_rate_inner_min.length) {
          IB_SEARCH_FILTER_FORM.find('[name="min_cap_rate"]').val(
            params.min_cap_rate
          );
        }

        if (ib_comm_rate_inner_max.length) {
          IB_SEARCH_FILTER_FORM.find('[name="max_cap_rate"]').val(
            params.max_cap_rate
          );
        }
        ...
        if (ib_comm_bsize_inner_min.length) {
          if (
            params.min_building_size != null &&
            params.min_building_size != ""
          ) {
        ...
        if (ib_comm_bsize_inner_max.length) {
          if (
            params.max_building_size != null &&
            params.max_building_size != ""
          ) {
